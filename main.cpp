...
void vir_kb_pressed()
{
	g_prev_vir_state.vir_kb_pressed = true;
}

void vir_kb_released()
{
	INPUT inputs[6] = {};
	ZeroMemory(inputs, sizeof(inputs));
	...
	SendInput(ARRAYSIZE(inputs), inputs, sizeof(INPUT));
	g_prev_vir_state.vir_kb_pressed = false;

	cout << "Displaying virtual keyboard (note: only works when running this app as Administrator!)" << endl;
}
	...
	while (true) {
		...
		if (g_is_enabled) {
			...
			if (g_js_connected) {
				...
				if (!g_prev_vir_state.vir_kb_pressed && state.buttons[GLFW_GAMEPAD_BUTTON_BACK] && state.buttons[GLFW_GAMEPAD_BUTTON_START])
					vir_kb_pressed();
				else if (g_prev_vir_state.vir_kb_pressed && (!state.buttons[GLFW_GAMEPAD_BUTTON_BACK] || !state.buttons[GLFW_GAMEPAD_BUTTON_START]))
					vir_kb_released();
